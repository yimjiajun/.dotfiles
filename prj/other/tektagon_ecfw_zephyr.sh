...
for re_install in {1..3}; do
  ...
  if [[ -d 'ecfwwork/zephyr_fork' ]]; then
  ...
  if [[ "$(west config zephyr.base-prefer)" == "configfile" ]]; then
  ...
  if [ -d $mec_cpg_zephyr_docs_path ]; then
    if ! [[ $(git -C $mec_cpg_zephyr_docs_path remote get-url origin) =~ .*"security/nightfury/opensourceprojects/cpgzephyrdocs".* ]]; then
      if [ -d ${mec_cpg_zephyr_docs_path}~ ]; then
  ...
  if [[ $mchp_series =~ .*[mM][eE][cC]15.* ]]; then
    ...
    if [[ -n $spi_gen ]]; then
      if [[ $(grep -c 'export EVERGLADES_SPI_GEN' "${HOME}/.$(basename ${SHELL})rc") -ne 0 ]]; then
    ...
    if [[ -n $spi_cfg ]]; then
      if [[ $(grep -c 'export EVERGLADES_SPI_CFG' "${HOME}/.$(basename ${SHELL})rc") -ne 0 ]]; then
  ...
  elif [[ $mchp_series =~ .*[mM][eE][cC]172.* ]]; then
    ...
    if [[ -n $spi_gen ]]; then
      if [[ $(grep -c 'export MEC172X_SPI_GEN' "${HOME}/.$(basename ${SHELL})rc") -ne 0 ]]; then
    ...
    if [[ -n $spi_cfg_path ]]; then
      if [[ $(grep -c 'export MEC172X_SPI_CFG' "${HOME}/.$(basename ${SHELL})rc") -ne 0 ]]; then
