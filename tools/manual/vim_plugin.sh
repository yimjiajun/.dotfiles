...
if ! [ -f "$calendar_crendiatials" ]; then
  ...
  display_info "link" "$calendar_crendiatials"

  if ! ln -sfr $calendar_crendiatials_localdata $calendar_crendiatials; then
    display_error "failed to link calendar credentials"
    exit 1
  fi
fi
