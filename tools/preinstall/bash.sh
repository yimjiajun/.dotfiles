...
if [[ $OSTYPE == "darwin"* ]]; then
  ...
  if [ -f "$mac_startup_bash_profile" ]; then
    ...
    if [ "$found_bashrc_in_profile" -eq 0 ]; then
      ...
      append_to_file_if_not_exists "$mac_startup_bash_profile" "source $HOME/.bashrc" || exit 1
    fi
  fi
fi

link_file "$common_data_path/.bash_aliases" "$HOME/.bash_aliases" || exit 1
link_file "$common_data_path/.bash_functions" "$HOME/.bash_functions" || exit 1

if ! [ -f "$common_data_path/.bash_setup" ]; then
  return 0
fi

append_to_file_if_not_exists "$HOME/.$(basename "$SHELL")rc" "source $HOME/.bash_${USER}" || exit 1
